    ...
    while (deCluster.length < numberOfCities && j < results.length) {
      ...
      if (j == 0) {
        ...
        for (let i = 0; i < deCluster.length; i++) {
          if (
            myFunctions.distance(
              results[j].coordinates[0],
              results[j].coordinates[1],
              deCluster[i].coordinates[0],
              deCluster[i].coordinates[1]
            ) < 50
          ) {
            addItem = false;
