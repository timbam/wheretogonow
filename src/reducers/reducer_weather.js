...
export default function(state = INITIAL_STATE , action) {
  switch (action.type) {
    ...
    case FETCH_WEATHER:
      ...
      const index = state.nearbyCitiesWeather.map(function(x) {return x._id; }).indexOf(action.payload.data._id);
      const newArray = index > -1 ?
          state.nearbyCitiesWeather.slice(0,index).concat(state.nearbyCitiesWeather.slice(index+1)) :
  ...
  pData.weatherdata.product.time.map((item, index) =>{
    ...
    if(previousDate != dFrom.getDate()) {
      ...
      if(weatherObj.symbol && weatherObj.maxTemperature) {
        ...
        if(weatherObj.symbol.number && Number(weatherObj.symbol.number) <= 4) weatherObj.weatherRating = Number(weatherObj.symbol.number) - 1;
        weatherObj.weatherRating += Math.abs(26-weatherObj.maxTemperature.value);
        if(weatherObj.precipitation) weatherObj.weatherRating += Number(weatherObj.precipitation.value)*15;
